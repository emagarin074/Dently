...
export async function recordPayment(
  ...
  if (!VALID_PAYMENT_METHODS.includes(method)) return { error: "Invalid payment method." }

  try {
    const billing = await prisma.billing.findFirst({ where: { id: billingId, clinicId: user.clinicId } })
    ...
    const status = newPaid >= total ? "FULLY_PAID" : newPaid > 0 ? "PARTIALLY_PAID" : "UNPAID"
...
export async function createBilling(clinicSlug: string, appointmentId: string, totalAmount: number) {
  const user = await requireAuth(clinicSlug)
  if (!user) return { error: "Unauthorized" }
