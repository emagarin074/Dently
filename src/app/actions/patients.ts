...
export async function updatePatient(clinicSlug: string, patientId: string, formData: FormData) {
  const user = await requireAuth(clinicSlug)
  if (!user) return { error: "Unauthorized" }
...
export async function saveHealthQuestionnaire(clinicSlug: string, patientId: string, formData: FormData) {
  const user = await requireAuth(clinicSlug)
  if (!user) return { error: "Unauthorized" }
  ...
  try {
    ...
    const data = {
      hasHypertension: formData.get("hasHypertension") === "true",
      hasDiabetes: formData.get("hasDiabetes") === "true",
      hasHeartDisease: formData.get("hasHeartDisease") === "true",
      ...
      previousDentalWork: (formData.get("previousDentalWork") as string) || null,
...
export async function addPatientNote(clinicSlug: string, patientId: string, content: string) {
  const user = await requireAuth(clinicSlug)
  if (!user) return { error: "Unauthorized" }

  if (!content || !content.trim()) return { error: "Note content is required." }
  ...
  try {
    ...
    await prisma.patientNote.create({ data: { patientId, content: content.trim() } })
    revalidatePath(`/clinic/${clinicSlug}/admin/patients/${patientId}`)
    return { success: true }
  } catch (error) {
