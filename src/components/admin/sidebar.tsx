...
import {
  ...
  FileText, BarChart3, Settings, UserCog, Stethoscope, QrCode, Menu, X
} from "lucide-react"
...
export function AdminSidebar({ clinicSlug, clinicName, userRole, userName }: Props) {
  const pathname = usePathname()
  const [mobileOpen, setMobileOpen] = useState(false)
  const base = `/clinic/${clinicSlug}/admin`

  const links = userRole === UserRole.DENTIST_ADMIN ? adminLinks : assistantLinks

  const NavContent = () => (
    <div className="flex flex-col h-full">
      <div className="px-4 py-5 border-b">
        <Link href={`/clinic/${clinicSlug}`} className="flex items-center gap-2">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {links.map((link) => {
          ...
          const isActive = link.href === "" ? pathname === base : pathname.startsWith(`${base}${link.href}`)
          return (
            <Link
              key={link.href}
              href={href}
      ...
      <div className="border-t px-4 py-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground">{userRole === UserRole.DENTIST_ADMIN ? "Dentist Admin" : "Assistant"}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 flex h-14 items-center justify-between border-b bg-white px-4">
        <Link href={`/clinic/${clinicSlug}`} className="font-semibold text-primary">{clinicName}</Link>
